...
App::put('/v1/users/:userId/labels')
    ...
    ->label('sdk.response.model', Response::MODEL_USER)
    ->param('userId', '', new UID(), 'User ID.')
    ->param('labels', [], new ArrayList(new Text(128, allowList: [...Text::NUMBERS, ...Text::ALPHABET_UPPER, ...Text::ALPHABET_LOWER]), APP_LIMIT_ARRAY_PARAMS_SIZE), 'Array of user labels. Replaces the previous labels. Maximum of 100 labels are allowed, each 128 characters long. Valid chars are a-z, A-Z, and 0-9.')
    ->inject('response')
    ->inject('dbForProject')
    ->inject('events')
    ->action(function (string $userId, array $labels, Response $response, Database $dbForProject, Event $events) {
        ...
        $user->setAttribute('labels', (array) $labels);
